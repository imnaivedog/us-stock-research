        ...
        conn.execute(
            text(
                ...
                INSERT INTO symbol_universe_changes (
                    symbol, change_type, reason, market_cap, pool, thesis_url
                )
                VALUES (
                    :symbol, :change_type, :reason, :market_cap, :pool, :thesis_url
                )
...
def upsert_universe_symbols(engine: Engine, rows: Sequence[dict[str, Any]]) -> int:
    ...
    defaults = {
        ...
        "filter_reason": None,
        "thesis_url": None,
        "thesis_added_at": None,
    }
    normalized = [{**defaults, **row} for row in normalized]
    with engine.begin() as conn:
        conn.execute(
            text(
                """
                ...
                VALUES (
                    ...
                    :thesis_url, COALESCE(:thesis_added_at, now())
                )
                ON CONFLICT (symbol) DO UPDATE SET
                    ...
                    thesis_added_at = COALESCE(
                        symbol_universe.thesis_added_at, EXCLUDED.thesis_added_at
                    ),
                    updated_at = now()
                """
            ),
