...
def load_macro_quotes(engine: Engine, trade_date: date) -> pd.DataFrame:
...
def upsert_macro_state(engine: Engine, result: MacroResult) -> None:
    sql = text(
        """
...
def run_macro(engine: Engine, trade_date: date) -> MacroResult:
    result = compute_macro_state(load_macro_quotes(engine, trade_date), trade_date)
