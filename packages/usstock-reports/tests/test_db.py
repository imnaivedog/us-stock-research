...
from usstock_reports import db


def test_load_dotenv_called(tmp_path, monkeypatch) -> None:
    env_file = tmp_path / ".env"
    env_file.write_text("REPORTS_DOTENV_VALUE=loaded\n", encoding="utf-8")
    monkeypatch.delenv("REPORTS_DOTENV_VALUE", raising=False)

    assert db._load_repo_dotenv(env_file) is True

    assert os.environ["REPORTS_DOTENV_VALUE"] == "loaded"
