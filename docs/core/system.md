...
| M signals | `signals_daily`, `signals_alerts`, `signals_sectors_daily`, `signals_themes_daily`, `signals_stocks_daily` |
...
| Table | Contract |
| --- | --- |
| `signals_daily` | 每日一行：`regime`, `regime_streak`, breadth fields, `sectors_top3`, `sectors_quadrant`, `themes_top3`, `macro_state` |
| `signals_alerts` | L2 breadth / risk alerts 与相关 daily warnings |
| `signals_sectors_daily` | 每 sector / day 一行：五维 score、`total_score`, `rank_relative`, `pct_5y`, `quadrant`, `multiplier` |
| `signals_themes_daily` | 每 theme / day 一行：`state`, `rank`, `total_score`, volume alert 与 core/diffusion change |
| `signals_stocks_daily` | 每 symbol / day 一行：`total_score`, `technical_score`, `fundamental_score`, `theme_bonus`, `sector_multiplier`, `rank`, `is_top5`, `entry_pattern`, `primary_sector`, `primary_theme` |
...
| L4 themes + stocks | `signals_daily.themes_top3`, `signals_themes_daily`, `signals_stocks_daily` |
| Macro | `signals_daily.macro_state` |
...
```text
total_score = (technical_score + fundamental_score + theme_bonus) * sector_multiplier
```

持久化行包含 `trade_date`, `symbol`, `total_score`, `technical_score`, `fundamental_score`, `theme_bonus`, `sector_multiplier`, `rank`, `is_top5`, `entry_pattern`, `primary_sector`, `primary_theme`, `as_of_date`。
...
- `watching` 只进入 YAML / universe mirror，不参与 A 池 signals 评分；用户补齐 thesis numbers 并确认后再改为 `active`。
