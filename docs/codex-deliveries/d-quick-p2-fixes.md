...
btc: "BTCUSD"
ief: "IEF"         # 7-10y treasury proxy.
us10y: "IEI"       # 3-7y yield-curve proxy; adjacent to IEF, not a duplicate holding.
us2y: "SHY"        # short-duration proxy; ^FVX is 402.
```
...
- P2.12 顺手处理结果: 保留 `ief: IEF` 与 `us10y: IEI`。本地记录的 ETF 期限语义为 IEF 7-10y、IEI 3-7y，属于相邻期限代理；yaml 已补充语义备注。
...
- FMP weight 实际返回格式: `0-100`。小批量 parquet 中字段为 `weightPercentage`，5 个 loader ETF 的 sum 约为 `100`。
...
- [ ] migration 脚本本地 forward + rollback DB 执行：尝试用本机 PostgreSQL 16 初始化临时库；`initdb` 失败，原因见偏离记录。
