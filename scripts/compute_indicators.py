...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dry-run", action="store_true", help="Compute rows without DB writes.")
...
def load_active_symbols(pg: PostgresClient) -> list[str]:
    sql = text("SELECT symbol FROM symbol_universe WHERE is_active IS TRUE ORDER BY symbol")
    with pg.engine.begin() as conn:
        return [str(row[0]).upper() for row in conn.execute(sql).all()]
...
def load_quote_history(
    ...
    params: dict[str, Any] = {"as_of": as_of, "lookback_rows": lookback_rows}
    ...
    sql = text(
        ...
        FROM (
            ...
            WHERE q.trade_date <= :as_of
              AND q.symbol IN ({", ".join(placeholders)})
        ) ranked
...
def indicator_rows_for_date(
    ...
    wanted = set(symbols)
    rows = indicators[
        (indicators["trade_date"] == as_of) & (indicators["symbol"].isin(wanted))
    ].copy()
...
def build_upsert_sql() -> str:
    ...
    update_cols = ", ".join(
        f"{column} = EXCLUDED.{column}" for column in INDICATOR_COLUMNS
    )
    return (
...
def upsert_daily_indicators(pg: PostgresClient, rows: list[dict[str, Any]]) -> None:
    if not rows:
        return
    with pg.engine.begin() as conn:
...
def log_symbol_failure(pg: PostgresClient, symbol: str, trade_date: date, message: str) -> None:
    try:
        with pg.engine.begin() as conn:
            conn.execute(
                text(
        ...
        logger.warning(f"Could not write alert_log for {symbol}: {exc}")
...
def compute_rows_for_symbols(
    ...
    for symbol in symbols:
        try:
            ...
            if pg is not None:
                log_symbol_failure(pg, symbol, as_of, str(exc))
    return rows, failed
...
def run_compute_indicators(
    ...
    if not symbols:
        raise RuntimeError("No symbols requested for indicator computation")
    logger.info(f"compute_indicators as_of={as_of}, symbols={len(symbols)}, dry_run={dry_run}")
    quotes = load_quote_history(pg, as_of, symbols)
    rows, failed = compute_rows_for_symbols(quotes, as_of, symbols, pg=pg)
    logger.info(f"compute_indicators computed_rows={len(rows)}, failed_symbols={len(failed)}")
    if not dry_run:
        upsert_daily_indicators(pg, rows)
...
def main() -> None:
    ...
    logger.info(
        f"compute_indicators finished in {time.monotonic() - started:.1f}s "
