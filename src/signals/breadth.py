...
def breadth_score(row: pd.Series, params: dict[str, Any]) -> int:
    ...
    score = (
        ...
        + mcclellan_normalized(float(row["breadth_mcclellan"]))
        * weights["mcclellan_normalized"]
    )
...
def detect_50ma_alerts(
    ...
    if len(recent) >= int(dull["consec_days"]) and (
        recent["breadth_pct_above_50ma_p2y"] >= float(dull["threshold_p2y"])
    ).all():
        alerts.append(
            Alert(
                trade_date,
...
def is_zweig_thrust(
    ...
    if recent.empty:
        return False
    return (
        recent["breadth_pct_above_50ma"].min() <= float(zweig["from_threshold"])
        and recent["breadth_pct_above_50ma"].iloc[-1] >= float(zweig["to_threshold"])
    )
